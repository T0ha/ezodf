...
#<office:text>
TEXT_PRELUDE = frozenset(OFFICE_FORMS + TABLE_TRACKED_CHANGES + TEXT_DECL + TABLE_DECL)
TEXT_CONTENT_STREAM = frozenset(TEXT_CONTENT_ELEMENTS + SHAPE + CHANGE_MARKS)
TEXT_CONTENT_PAGED = frozenset(PAGE_SEQUENCE)
TEXT_EPILOGUE = frozenset(TABLE_FUNCTIONS)
...
TABLE_PRELUDE = frozenset([
    ...
    CN("office:forms"),
    CN("table:shapes")]),
TABLE_CONTENT = frozenset(TABLE_COLUMNS + TABLE_ROWS)
TABLE_EPILOGUE = []

# <office:spreadsheet>
SPREADSHEET_PRELUDE = frozenset(TABLE_TRACKED_CHANGES + TEXT_DECL + TABLE_DECL)
SPREADSHEET_CONTENT = frozenset([CN("table:table"),])
SPREADSHEET_EPILOGUE = frozenset(TABLE_FUNCTIONS)
